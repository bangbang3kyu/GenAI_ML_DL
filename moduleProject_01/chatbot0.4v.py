...
for idx, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        ...
        # Assistant 답변은 Markdown 파일로 다운로드할 수 있게 한다.
        # 보고서 답변을 Notion 등에 옮길 때 사용한다.
        if message["role"] == "assistant":
            st.download_button(
                label="답변 다운로드",
                data=message["content"],
                file_name=f"ids_chatbot_answer_history_{idx}.md",
                mime="text/markdown",
    ...
    with st.chat_message("assistant"):
        with st.spinner("누구보다 열심히 찾아보는중..."):
            try:
                ...
                # 방금 생성된 Assistant 답변을 Markdown 파일로 다운로드한다.
                st.download_button(
                    label="답변 다운로드",
                    data=answer,
